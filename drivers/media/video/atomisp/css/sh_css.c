...
{
	sh_css_params_set_shading_table(NULL);
	...
	my_css.shading_table_update = true;
}
		      ...
		      const struct sh_css_binary *binary)
{
	...
	if (*target_table) {
		sh_css_params_set_shading_table(NULL);
		sh_css_shading_table_free(*target_table);
		*target_table = NULL;
	}
		     ...
		     struct sh_css_frame *out_frame)
{
	...
	if (err != sh_css_success)
		return err;
	if (my_css.shading_table_update) {
		prepare_shading_table(
				&my_css.preview_settings.shading_table,
				preview_binary);
		sh_css_params_set_shading_table(
				my_css.preview_settings.shading_table);
		my_css.shading_table_update = false;
	}
		     ...
		     struct sh_css_frame *vf_frame)
{
	...
	if (err != sh_css_success)
		return err;
	if (my_css.shading_table_update && sc_binary) {
		prepare_shading_table(
				&my_css.capture_settings.shading_table,
				sc_binary);
		sh_css_params_set_shading_table(
