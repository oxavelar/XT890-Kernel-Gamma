...
static struct usb_string fastboot_string_defs[] = {
	[STRING_INTERFACE].s = "Motorola Fastboot Interface",
	{  /* ZEROES END LIST */ },
};
...
static struct usb_gadget_strings adb_string_table = {
	...
	.strings =              adb_string_defs, /* default */
};
...
adb_function_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	adb_string_table.strings[STRING_INTERFACE].id = 0;
...
static int adb_bind_config(struct usb_configuration *c)
{
	...
	if (status >= 0) {
		...
		adb_string_table.strings = fastboot ?
			fastboot_string_defs : adb_string_defs;
		if (adb_string_table.strings[STRING_INTERFACE].id == 0) {
			adb_string_table.strings[STRING_INTERFACE].id = status;
		}
	}
