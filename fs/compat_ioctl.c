...
static void compat_ioctl_dump_info(struct file *filp, unsigned int fd,
		unsigned int cmd, unsigned long arg)
{
	char buf[10];
	char *fn = "?";
	...
	if (!isprint(buf[1]))
		sprintf(buf, "%02x", buf[1]);
	compat_printk("ioctl32_dump(%s:%d): cmd fd(%d) "
			"cmd(%08x){t:%s;sz:%u} arg(%08x) on %s\n",
			current->comm, current->pid,
			(int)fd, (unsigned int)cmd, buf,
			(cmd >> _IOC_SIZESHIFT) & _IOC_SIZEMASK,
				...
				unsigned long arg)
{
 ...
 do_ioctl:
	...
	/* 64bit debugging
	static int cnt_temp;
	if ((error != -ENOIOCTLCMD) && (++cnt_temp <= 500))
		compat_ioctl_dump_info(filp, fd, cmd, arg);
	*************** */
