...
/* A boolean variable, that is set, when wants to set SW_AP_RESET flag
   to OSNIB */
static int sw_ap_reset_flag;
				     ...
				     unsigned long what, void *data)
{
	...
	if (!fastboot || sw_ap_reset_flag) {
		pr_warn("[SHTDWN] %s, set AP reset flag\n", __func__);
		ret_ipc = intel_scu_ipc_write_osnib_byte(
				OSNIB_OFFSET + OSNIB_PANIC_OFFSET,
				OSNIB_PU_REASON_SW_AP_RESET);
		if (ret_ipc < 0)
			pr_err("%s cannot set AP reset flag in OSNIB\n", __func__);
	}
	...
	if (what != SYS_RESTART) {
		if (what == SYS_HALT || what == SYS_POWER_OFF) {
			...
			pr_warn("[SHTDWN] %s, default to Android\n", __func__);
...
module_param(sw_ap_reset_flag, int, 0644);
MODULE_PARM_DESC(sw_ap_reset_flag,
		"Variable to be set by user space"
		" when need to set SW_AP_RESET flag in OSNIB");
