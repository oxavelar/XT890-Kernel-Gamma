...
/*
 * This writes the fastboot mode in the OSNIB (factor)
 */
int intel_scu_ipc_write_osnib_fb_mode(u8 flag)
{
	return intel_scu_ipc_write_osnib(&flag, 1, OSNIB_FBMODE_OFFSET);
}
EXPORT_SYMBOL_GPL(intel_scu_ipc_write_osnib_fb_mode);

/*
 * This writes the bos resync reason in the OSNIB (factor and avoid any overlap)
 */
int intel_scu_ipc_write_osnib_bos(u8 bos)
{
	return intel_scu_ipc_write_osnib(&bos, 1, OSNIB_BOS_OFFSET);
}
EXPORT_SYMBOL_GPL(intel_scu_ipc_write_osnib_bos);
...
/*
 * This writes the one byte to  the OSNIB (factor and avoid any overlap)
 */
int intel_scu_ipc_write_osnib_byte(u8 offset, u8 data)
{
	return intel_scu_ipc_write_osnib(&data, 1, offset-OSNIB_OFFSET);
}
EXPORT_SYMBOL_GPL(intel_scu_ipc_write_osnib_byte);

/*
 * This reads the one byte from the OSNIB (factor)
 */
int intel_scu_ipc_read_osnib_byte(u8 offset, u8 *data)
{
	return intel_scu_ipc_read_osnib(data, 1, offset-OSNIB_OFFSET);
}
EXPORT_SYMBOL_GPL(intel_scu_ipc_read_osnib_byte);

/*
 * This reads the fastboot mode from the OSNIB (factor)
 */
int intel_scu_ipc_read_osnib_fb_mode(u8 *flag)
{
	return intel_scu_ipc_read_osnib(flag, 1, OSNIB_FBMODE_OFFSET);
}
EXPORT_SYMBOL_GPL(intel_scu_ipc_read_osnib_fb_mode);

/*
 * This reads the bos sync reason from the OSNIB (factor)
 */
int intel_scu_ipc_read_osnib_bos(u8 *bos)
{
	return intel_scu_ipc_read_osnib(bos, 1, OSNIB_BOS_OFFSET);
}
EXPORT_SYMBOL_GPL(intel_scu_ipc_read_osnib_bos);
...
{
#ifdef DUMP_OSNIB
	u8 rr, resetirq1, resetirq2, wd, alarm, wakesrc, *ptr, bos;
	u8 panic;
	...
	intel_scu_ipc_read_osnib(&panic, 1, OSNIB_PANIC_OFFSET);
	...
	intel_scu_ipc_read_osnib_bos(&bos);
	pr_warn("[BOOT] RR=0x%02x WD=0x%02x ALARM=0x%02x PANIC=0x%02x(osnib)\n",
		rr, wd, alarm, panic);
	pr_warn("[BOOT] WAKESRC=0x%02x RESETIRQ1=0x%02x RESETIRQ2=0x%02x "
		"(osnib)\n",
		wakesrc, resetirq1, resetirq2);
	pr_warn("[BOOT] BOS=0x%02x (osnib)\n", bos);
#endif
