...
/* Panel register access */
struct psb_drm_panel_reg_access {
	u8 address;
	u8 use_hs_mode;
	size_t buffer_size;
	void __user *buffer;
...
#define DRM_IOCTL_PANEL_REG_READ \
	DRM_IOWR(DRM_COMMAND_BASE + DRM_PANEL_REG_READ, \
		struct psb_drm_panel_reg_access)

#define DRM_IOCTL_PANEL_REG_WRITE \
	DRM_IOW(DRM_COMMAND_BASE + DRM_PANEL_REG_WRITE, \
		struct psb_drm_panel_reg_access)
...
static int psb_dpu_dsr_off_ioctl(struct drm_device *dev, void *data,
				 struct drm_file *file_priv);
static int psb_drm_panel_reg_read_ioctl(struct drm_device *dev, void *arg,
					struct drm_file *file_priv);
static int psb_drm_panel_reg_write_ioctl(struct drm_device *dev, void *arg,
					struct drm_file *file_priv);
...
static struct drm_ioctl_desc psb_ioctls[] = {
	...
	PSB_IOCTL_DEF(DRM_IOCTL_PSB_VSYNC_SET, psb_vsync_set_ioctl, DRM_AUTH | DRM_UNLOCKED),

#ifdef CONFIG_DRM_PANEL_REG_ACCESS
	PSB_IOCTL_DEF(DRM_IOCTL_PANEL_REG_READ,
		psb_drm_panel_reg_read_ioctl, 0),
	PSB_IOCTL_DEF(DRM_IOCTL_PANEL_REG_WRITE,
		psb_drm_panel_reg_write_ioctl, 0),
#endif
...
bool mrst_get_vbt_data(struct drm_psb_private *dev_priv)
{
	...
	printk(KERN_ALERT"PanelID:%d\n", PanelID);
		...
		/* TODO: Need to implement runtime detection of panel */
		dev_priv->panel_id = SMD_QHD_AMOLED_430_CMD;
		PanelID = SMD_QHD_AMOLED_430_CMD;
		printk(KERN_ALERT"%s: AMOLED CMD Panel\n", __func__);
#endif
	return true;
}
...
{
	int r = 0;
	struct psb_drm_panel_reg_access *r_access = arg;
	u8 *buf;

	/*reject requests from non-mdfld platforms*/
	if (!IS_MDFLD(dev)) {
		DRM_INFO("Not Medfield platform!");
		return -EOPNOTSUPP;
	}

	buf = kmalloc(r_access->buffer_size, GFP_KERNEL);
	if (!buf) {
		DRM_ERROR("Failed to allocate reg read buffer\n");
		return -ENOMEM;
	}

	r = mdfld_dsi_dbi_panel_reg_read(dev, r_access->address,
					r_access->buffer_size,
					buf, r_access->use_hs_mode);
	if ((r == 0) && (copy_to_user(r_access->buffer,
			buf, r_access->buffer_size)))
		r = -EFAULT;
					...
					struct drm_file *file_priv)
{
	...
	return mdfld_dsi_dbi_panel_reg_write(dev, r_access->address,
					r_access->buffer_size,
					r_access->buffer,
					r_access->use_hs_mode);
}
#endif
...
static int mdfld_display_regs_info(struct seq_file *m, void *data)
{
	...
	if (ospm_power_using_hw_begin(OSPM_DISPLAY_ISLAND,
					OSPM_UHB_FORCE_POWER_ON)) {
		mdfld_dsi_dsr_forbid(dsi_config);
		seq_printf(m, "Display registers:\n");
		...
		seq_printf(m, "\tDPLL:\n");
		for (i = 0xf000; i < 0xf074; i += 4)
			seq_printf(m, "\t\t reg(0x%x) = 0x%x\n",
				i, REG_READ(i));

		seq_printf(m, "\tOVERLAY A:\n");
		for (i = 0x30000; i < 0x30203; i += 4)
			seq_printf(m, "\t\t reg(0x%x) = 0x%x\n",
				i, REG_READ(i));
...
static struct drm_info_list mdfld_debugfs_list[] = {
	{"mdfld_display_regs", mdfld_display_regs_info, 0},
	{"dynamic_gamma", smd_qhd_amoled_430_gamma_dbg_dump, 0},
	{"dynamic_gamma_csv", smd_qhd_amoled_430_gamma_dbg_dump_csv, 0}
};
...
#if defined(CONFIG_DEBUG_FS)
	drm_debugfs_create_files(mdfld_debugfs_list,
...
static void __exit psb_exit(void)
{
	...
	|| defined(CONFIG_SUPPORT_SMD_QHD_AMOLED_COMMAND_MODE_DISPLAY)
