...
#define CHRINT1_MASK MSIC_BATT_CHR_VINREGMINT_MASK
...
#define CHR_SPCHRGER_WEAKVIN_LVL2	0x05
...
#define BATT_OVERVOLTAGE_CUTOFF_VOLT	4500	/* 4500mV */
#define BATT_OVERVOLTAGE_CUTOFF_VOLT_HYST	420	/* 420mV */
...
typedef union {
	...
	uint32_t    raw32[2];
	struct {
		uint64_t    familyCode:8;
		int64_t    serialNum:32;
		uint64_t    manufactureId:16;
		uint64_t    crc:8;
	} field;
} RegisNum;

typedef union {
	uint8_t     raw[NUM_PAGES * BYTES_PER_PAGE];
	uint32_t    raw32[NUM_PAGES * BYTES_PER_PAGE/4];
	struct {
...
/* SFI table entries structure.*/
struct msic_batt_sfi_prop_oem1 {
	RegisNum RegistrationNum;
	EpromArray EpromData;
} __packed;
