...
#define MAX_I2C_SPURIOUS_INTERRUPT	30
/*
...
irqreturn_t i2c_dw_isr(int this_irq, void *dev_id)
{
	...
	if (dev->dev->power.runtime_status != RPM_ACTIVE) {
		...
		counter++;
		/* If get MAX spurious interrupts, that means dw-i2c device
		 * meet a big issue and can not be recovered.
		 * Trigger a BUG to catch this situation.
		 */
		if (counter > MAX_I2C_SPURIOUS_INTERRUPT)
			BUG();
		else
			WARN_ON(1);
		pm_runtime_put_autosuspend(dev->dev);
	...
	if (!enabled || !(stat & ~DW_IC_INTR_ACTIVITY)) {
		counter++;
		/* Trigger a BUG to catch the spurious interrupts issue */
		if (counter > MAX_I2C_SPURIOUS_INTERRUPT)
			BUG();
		pm_runtime_put_autosuspend(dev->dev);
		return IRQ_NONE;
