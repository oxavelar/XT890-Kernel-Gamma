...
static void emmc_panic_notify_add(void)
{
	...
	u8 data = OSNIB_PU_REASON_AP_KERNEL_PANIC;
	int ret;
	...
	if (fastboot) {
		ret = intel_scu_ipc_write_osnib(&data, 1, OSNIB_PANIC_OFFSET);
		if (ret < 0) {
			printk(KERN_ERR "%s: Write OSNIB panic flag failed!\n", __func__);
		}
	}
...
static void emmc_ipanic_write_app_thread_func(void)
{
	struct emmc_ipanic_data *ctx = &drv_ctx;
	struct mmc_emergency_info *emmc = ctx->emmc;
	int size_written;
	...
	thread_sector_count =
	    emmc_ipanic_write_console(emmc,
				      log_offset[IPANIC_LOG_APP_THREADS] +
				      log_len[IPANIC_LOG_APP_THREADS],
				      &size_written);
	if (thread_sector_count < 0) {
		printk(KERN_EMERG "Error writing threads to panic log! (%d)\n",
		       log_len[IPANIC_LOG_APP_THREADS]);
	...
	log_size[IPANIC_LOG_APP_THREADS] += size_written;
	log_len[IPANIC_LOG_APP_THREADS] += thread_sector_count;
...
static void emmc_ipanic_write_calltrace(struct mmc_emergency_info *emmc,
					int log, unsigned long threads_filter)
{
	log_offset[log] = log_offset[log - 1] + log_len[log - 1];
	/*
	...
	if (threads_filter & SHOW_KTHREADS)
		...
		func_stream_emmc = emmc_ipanic_write_app_thread_func;

	if ((get_trace_buf_size() < (SZ_512K + 1)) &&
			(threads_filter & SHOW_KTHREADS))
		ftrace_dump(1);
	show_state_filter(0, threads_filter);
				       ...
				       unsigned long action, void *data)
{
	...
	case BUS_NOTIFY_BOUND_DRIVER:
		...
		if (emmc->disk_device) {
			...
			disk_part_iter_init(&piter, emmc->disk,
